...
- **Settings drawer** — ≡ or Ctrl+, : four themes (ember/paper/phosphor/graphite), text size, voice on/off + level, wake word, mic mute, always-on-top, new chat/restart. Preferences persist in `ui-config.json`.
...
- **პარამეტრების პანელი** — ≡ ან Ctrl+, : ოთხი თემა (ember/paper/phosphor/graphite), ტექსტის ზომა, ხმა ჩართვა/გამორთვა + სიმაღლე, გამოღვიძების სიტყვა, მიკროფონის დადუმება, ყოველთვის-ზემოთ, ახალი საუბარი/გადატვირთვა. პარამეტრები ინახება `ui-config.json`-ში.
