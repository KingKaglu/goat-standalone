...
- **Two-brain model router with token economy** — a fast talking brain (Claude Sonnet) handles conversation; work verbs route straight to the full working brain (Claude's top model). Heavy sessions stick to one model (prompt-cache friendly) and self-compact past 60k tokens, so long sessions stay affordable.
...
- **Georgian mode** — GOAT answers in Georgian, spoken with Microsoft's ka-GE neural voice. Voice *input* stays English (local Whisper's Georgian was measured unusable — romanization/hallucinations); type Georgian freely. `GOAT_STT_KA=on` re-enables the experimental Georgian hearing if you have a better model/machine.
...
2. **Thinking**: text goes to Claude via the **Claude Agent SDK**. Chat runs on the cheap/fast model; obvious work (fix/build/search/run…) routes straight to the full model, and anything else escalates itself. Past 25k tokens of context the session stops switching models (prompt cache is per-model); past 60k it compacts itself and carries on. There is **no API key in this repo** — the SDK uses your own local Claude Code sign-in.
...
- **ორტვინიანი როუტერი ტოკენების ეკონომიით** — საუბარს სწრაფი ტვინი (Claude Sonnet) უძღვება; სამუშაო ზმნები პირდაპირ სრულ „მუშა ტვინთან" (Claude-ის უმძლავრესი მოდელი) მიდის. მძიმე სესია ერთ მოდელზე რჩება (ქეშისთვის) და 60k ტოკენის შემდეგ თვითონვე იკუმშება — გრძელი სესიები იაფი რჩება.
...
- **ქართული რეჟიმი** — GOAT ქართულად გპასუხობს, Microsoft-ის ka-GE ნეირონული ხმით. ხმოვანი *შეყვანა* ინგლისურად რჩება (ლოკალური Whisper-ის ქართული გაზომვით უვარგისი აღმოჩნდა — ლათინიზაცია/ჰალუცინაციები); ქართულად წერა თავისუფლად შეგიძლია. `GOAT_STT_KA=on` ექსპერიმენტულ ქართულ სმენას რთავს, თუ უკეთესი მოდელი/კომპიუტერი გაქვს.
